...
class TunService : VpnService(), CoroutineScope by CoroutineScope(Dispatchers.Default) {
    ...
    private fun TunModule.open() {
        ...
        val device = with(Builder()) {
            ...
            addAddress("7.114.19.1", 30)

            if (store.clashrayNetAdditionalAddrSuffix.length > 0) {
                addAddress("172.19.0." + store.clashrayNetAdditionalAddrSuffix, 32)
                addAddress("7.114.19." + store.clashrayNetAdditionalAddrSuffix, 32)
            }
